...
class MemoryTest extends FreeSpec with ChiselScalatestTester {
  ...
  "MaskedSeqMem should read and write with mask" in {
    test(new Memory(heapSize, chCount, cWid, nWid, rWid)) { dut =>
    ...
    test(new Memory(heapSize, chCount, cWid, nWid, rWid)) { dut =>
      ...
      for(i <- 0 until 1000){
        ...
        if(refIDs.contains(searchKey)){
          assert(dut.srch.res.peek.litValue == refIDs.indexOf(searchKey),clue)
        }else{
