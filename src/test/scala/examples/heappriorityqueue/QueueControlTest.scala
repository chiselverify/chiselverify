...
/**
  ...
  */
class QueueControlTest extends AnyFreeSpec with ChiselScalatestTester with Matchers {
  ...
  def tester(c: QueueControl, debugLvl: Int, testRuns: Int)(implicit parameters: PriorityQueueParameters): Unit = {
    ...
    val model = new Behavioural(parameters.size, order)(superCycleWidth, cycleWidth, referenceIdWidth)
    ...
    def test(values: Seq[Seq[Int]]): Unit = {
      for (poke <- values) {
        ...
        cr.sample()
        c.io.head.prio.superCycle.peek().litValue should equal (model.mem(0)(0))
        c.io.head.prio.cycle.peek().litValue should equal (model.mem(0)(1))
        c.io.head.refID.peek().litValue should equal (model.mem(0)(2))
        // Scala 2.13 has deprecated .deep, but we only care about equality
        (java.util.Objects.deepEquals(model.mem.slice(1, model.mem.length), dut.mem.flatten)) should be (true)
      }
    }

    //////////////////////////////////////////////////////testing/////////////////////////////////////////////////////////
    test(Seq.fill(testRuns)(randomPoke()))
    ...
    println(s"${"=" * 20}Report${"=" * 20}\n" +
        s"Heapsize = ${parameters.size}, children count = $order\n" +
...
class QueueControlWrapper(dut: QueueControl, debug: Int)(implicit parameters: PriorityQueueParameters) {
  ...
  def pokeID(id: Int): Unit = dut.io.cmd.refID.poke(id.U)
  ...
  def getMem(): String = {
    s"${dut.io.head.prio.superCycle.peek().litValue}:${dut.io.head.prio.cycle.peek().litValue}:${dut.io.head.refID.peek().litValue} | ${mem.map(_.map(_.mkString(":")).mkString(", ")).mkString(" | ")}"
  }

  def getRmPrio(): Array[Int] = Seq(dut.io.cmd.rm_prio.superCycle, dut.io.cmd.rm_prio.cycle).map(_.peek().litValue.toInt).toArray

  def getSuccess(): Boolean = !dut.io.cmd.result.peek().litToBoolean
  ...
  def compareWithModel(arr: Array[Array[Int]]): Boolean = {
    ...
    // Scala 2.13 has deprecated .deep, but we only care about equality
    res &= java.util.Objects.deepEquals(arr.slice(1, arr.length), mem.flatten)
    return res
  }

  def setDebugLvl(lvl: Int): Unit = debugLvl = lvl
}
