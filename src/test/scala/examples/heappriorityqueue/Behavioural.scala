...
object Behavioural {
  ...
  def memToString(arr: Array[Array[Int]], k: Int): String = {
    s"${arr(0).mkString(":")} | ${arr.slice(1, arr.length).sliding(k, k).toArray.map(_.map(_.mkString(":")).mkString(", ")).mkString(" | ")}"
  }
}
...
class Behavioural(size: Int, chCount: Int)(cWid: Int, nWid: Int, rWid: Int) {
  var mem = Array.fill(size)(Array(Math.pow(2, cWid).toInt - 1, Math.pow(2, nWid).toInt - 1, Math.pow(2, rWid).toInt - 1))
  var heapSize = 0
  ...
  def heapifyDown(i: Int): Boolean = {
    ...
    for (child <- children) {
      if (child < heapSize && (mem(smallest)(0) > mem(child)(0) || (mem(smallest)(0) == mem(child)(0) && mem(smallest)(1) > mem(child)(1)))) {
        smallest = child
      }
    }
    if (smallest != i) {
  ...
  def insert(arr: Array[Array[Int]]): Unit = arr.foreach(elem => insert(elem(0), elem(1), elem(2)))
  
  def insert(poke: Seq[Int]): Boolean = insert(poke(1), poke(2), poke(3))
  ...
  def incHeapSize(n: Int): Unit = heapSize += n

  def getHead: Seq[Int] = Seq.tabulate(3)(i => mem(0)(i))

  def getMem(): Seq[Seq[Int]] = mem.map(_.toSeq).toSeq

  def printMem(style: Int = 1): Unit = {
    if (style == 0) println(s"Model: ${mem(0).mkString(":")}\n${mem.slice(1, mem.length).sliding(chCount, chCount).toArray.map(_.map(_.mkString(":")).mkString(", ")).mkString("\n")}")
    else if (style == 1) println(s"Model: ${mem(0).mkString(":")} | ${mem.slice(1, mem.length).sliding(chCount, chCount).toArray.map(_.map(_.mkString(":")).mkString(", ")).mkString(" | ")}")
  }
}
