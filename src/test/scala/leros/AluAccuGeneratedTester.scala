...
class AluAccuGeneratedTester(dut: AluAccuGenerated) extends PeekPokeTester(dut) {
...
class GeneratedAluTesterSpec extends FlatSpec with Matchers {
  "AluAccu" should "pass" in {
    iotesters.Driver.execute(Array("--backend-name", "verilator"),() => new AluAccuGenerated){
      c => new AluAccuGeneratedTester(c)
    }
  }
}