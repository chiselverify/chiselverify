...
object GlobalCoverage {
  ...
  class QueryableCoverageDB {
    ...
    private val portToValueCycleMap = new mutable.ArrayBuffer[(String, (BigInt, BigInt))]()
    // Contains the registers range for a given port
    private val portToRangeMap = new mutable.HashMap[String, Range]()
    // Stores the names of all ports
    private val portNameToNameMap = new mutable.HashMap[String, String]()

    // Caches
    private val nHitsCache = new mutable.HashMap[(String, Int, Int, Int), BigInt]()
    // (id, expectedHits) -> Coverage
    private val coverageCache = new mutable.HashMap[(String, Int), Double]()
    ...
    def getHits(id: String, range: Option[Range] = None): BigInt = {
      ...
      val portToHitsMap = new mutable.HashMap[String, List[BigInt]]()

      // Get range
      val r = range.getOrElse(portToRangeMap.get(id) match {
        case Some(ran) => ran
        case _ => throw new IllegalArgumentException(s"$id must be registered before use!")
      })
    ...
    def getCoverage(id: String, hits: BigInt, range: Option[Range] = None, expected: Option[Int] = None): Double = {
      val rangeSize = range.getOrElse(portToRangeMap.get(id) match {
        case Some(r) => r
        case _ => throw new IllegalArgumentException(s"$id is not registered")
      }).size
    ...
    def register(port: (String, Data), range: Range): Unit = {
      ...
      portToRangeMap.get(port._1) match {
        ...
        case _ => portToRangeMap.update(port._1, range)
      }
    }
  }
  ...
  class QueryableCoverage[T <: Module](val dut: T) {
    ...
    ports.foreach (p => { db.register(p, DefaultBin.defaultRange(p._2)) })
