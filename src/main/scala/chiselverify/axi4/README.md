...
This folder includes implementations of AXI4 master interface definitions and a small framework providing support for all of the defined transactions from the AXI master's point of view in the protocol.

## TODO
- Fix AXI4-Lite support in `FunctionalMaster` (perhaps change some inheritance-related things) - it is not currently supported fully - the `FunctionalMaster` class will fail if created with a `SlaveLite` object!
