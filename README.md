...
2. `CoverGroup`s can then be created by using the `register` method of the coverage reported. This takes as parameter a list of `CoverPoints` that contain a `port` that will be sampled, a `portName` that will be shown in the report and a `List[Bins]` created from a name and a scala range.  
